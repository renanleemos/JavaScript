console.log("Meu primeiro programa: Trabalhando com variáveis");

const idade = 22;

console.log("Minha idade é " + idade);
console.log(idade + 2);

const idadeSomada = idade + 2;
console.log(idadeSomada);

//Atalho importante. Shift + Alt + Seta para baixo reescreve a linha anteiror.

//O uso de crases (Template string) na programação são strings que permitem expressões embutidas, ou seja, sem ser necessário você concatenar suas string e variáveis.